...
import type { TutorSessionSummary, TutorSessionSummaryStats } from '@/types/Tutor/tutor'
import { tutorHistoryDetailPath } from '@/utils/paths'

interface TutorTodayResultCardProps {
  session: TutorSessionSummary
  initialSummary?: TutorSessionSummaryStats | null
}

export function TutorTodayResultCard({
  session,
  initialSummary,
}: TutorTodayResultCardProps) {
  const { t } = useTranslation('tutor')
  const isCompleted = session.status === 'COMPLETED'

  // Fetch full detail if initialSummary is not already provided
  const { data: detailData, isPending, isError, refetch } = useTutorSessionDetailQuery(
    session.id,
    !initialSummary,
  )

  const summary = initialSummary ?? detailData?.summary
  ...
  if (isPending && !summary) {
    return <LoadingState minHeight={240} paper={false} />
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {t('history.viewDetail', 'Xem chi tiết bài test')}
        </Button>
      </Stack>
    </Paper>
  )
